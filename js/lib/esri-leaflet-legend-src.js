...
EsriLeaflet.LegendControl = L.Control.extend({
  ...
  _load: function() {
    auxoptions = this._layers[0].options.layers;
  ...
  _onLoad: function(error, legend) {
    if (!error) {
      ...
      for (var i = 0, len = legend.layers.length; i < len; i++) {
        ...
        var legendsHtml = '';
        console.log(layer);
        if (layer.layerName=="SERV_IMG_SUSCEP_INUN_FLUVIAL") {
          layer.layerName="Susceptibilidad de Inundación Fluvial"
          layer.legend[0].label = "  Muy bajo o nulo";
          layer.legend[1].label = "  Bajo";
          layer.legend[2].label = "  Moderado";
          layer.legend[3].label = "  Alto";
        }
        if (auxoptions.includes(layer.layerId)) {
      ...
      console.log(legendsHtml);
      console.log(this.options);
      var namePais = '';
      switch (this.options.pais) {
        case 'colombia':
          namePais = 'Colombia';
          break;
        case 'peru':
      ...
      $("#legendpane").append('<div class="content-leyendpane" id="legend_'+this.options.pais+'_'+this.options.id+'">'+ '<h5><b>'+paises[this.options.pais]["arrayCapas"][this.options.id]["name"]+' ('+namePais+')</b></h5>' + layersHtml + '<div class="b-border"></div>' +'</div>');
      $("#content-legend_"+this.options.pais+'_'+this.options.id).append('<div class="legend_capa">' + layersHtml + '</div>');
    }
  },
